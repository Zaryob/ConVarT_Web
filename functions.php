        ...
        foreach ($spemudGeneIdList as $key => $value) {
            ...
            for ($i = 0; $i < count($geneIdList); $i++) {
                ...
                while ($row_prots = mysqli_fetch_assoc($queryProteinIds)) {
                    ...
                    $spemud_radio_button = "<div class='convart-radio'><input id='$key-$temp_convart_ids' name='$key' value='$temp_convart_ids' type='checkbox' /> <label style='color:$colors[$colId];' for='$key-$temp_convart_ids'>GeneID: $tempGeneIds | $temp_gene_symbol | $temp_prot_ids | $variation_count</label></div>";
